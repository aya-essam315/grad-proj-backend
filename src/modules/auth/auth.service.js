...
export const logIn = asyncHandler(async (req,res,next)=>{
    ...
    if(!user.isConfirmed) return next(new Error("Email not confirmed", {cause:401}));
...
export const resetPassword = asyncHandler(async(req,res,next)=>{
    ...
    if(latestOtp.codeType != "forgetPassword")  return next(new Error("not allowed", {cause:401}));
    if(!compareValues({data:OTP, hashedData:latestOtp.code})) return next(new Error("invalid otp", {cause:401})); 
